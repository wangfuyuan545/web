function changedate() {
    ...
    var value2 = addDate(value1,10);

    $(document).ready(function (e) {
        SetProgressTime(null, value1+" 0:00:00", value2+" 0:00:00")
    });
}
...
var _speed = 1000*60*60;
var myfun;//执行方法，当前时间为参数
function getSpeed(){
  var timepicker=document.getElementById('timepicker');
  _speed=timepicker.value*1000;
}
function SetProgressTime(fun, startTime, endTime) {
    ...
    var begintime = Year+"-"+Month+"-"+currentDate+"-"+Hours;
    ...
    $("#scroll_Thumb").html(begintime);

    var url ='jsonfile/'+ begintime+'.json';
...
var ScrollBar = {
    ...
    Value: function () {
        ...
        $("#scrollBarBox").click(function (event) {
            ...
            var indexStart2 = SetTime(ScrollBar.value);
            SetInterval(ScrollBar.value);
            _index = ScrollBar.value;

            map.removeLayer(velocityLayer);
            var url ='jsonfile/'+ indexStart2+'.json';
...
function SetTime(value) {
    ...
    var indexStart2 = Year+"-"+month+"-"+currentDate+"-"+Hours;
    ...
    if ($(img).attr("title") == "暂停") {
        ...
        _mProgressTimer = window.setInterval(function () {
            if (_index <= ScrollBar.maxValue) {
                ...
                var indexStart3=SetTime(_index); map.removeLayer(velocityLayer);
                var url ='jsonfile/'+ indexStart3+'.json';
                $.getJSON(url, function(data) {
                    velocityLayer = L.velocityLayer({
                        displayValues: true,
                        displayOptions: {
